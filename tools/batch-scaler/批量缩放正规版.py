...
# -*- coding: utf-8 -*-
"""
SolidWorks 批量处理脚本 - 修复文件选择版 + 可调整缩放尺寸
"""
...
def set_scale_factor():
    ...
    while True:
        try:
            user_input = input("请输入缩放倍数 (默认 1.0)：").strip()
            
            # 默认值为 1.0
            if not user_input:
                scale = 1.0
                print(f"使用默认缩放倍数: {scale}")
                return scale
            ...
            if scale > 100:
                confirm = input(f"警告：缩放倍数 {scale} 很大，确认吗？(y/n)：").strip().lower()
                if confirm != 'y':
                    continue
            
            print(f"已设置缩放倍数: {scale}")
            return scale
            
        except ValueError:
            print("错误：请输入有效的数字（如 1.5, 0.8, 2.0）")
        except Exception as e:
            print(f"输入错误: {safe_str(e)}")
...
CONFIG = {
    ...
    'scale_factor': 1.0,  # 将在运行时通过 set_scale_factor() 设置
...
class SolidWorksBatchProcessor:
    ...
    def scale_model(self):
        ...
        try:
            ...
            print(f"    应用缩放倍数: {self.scale_factor}")
            ...
            for i, body in enumerate(bodies_list):
                try:
                    ...
                    try:
                        # 获取特征管理器
                        feature_manager = self.sw_model.FeatureManager
                        
                        # 创建缩放特征
                        # 使用 Scale 方法
                        scale_feature = feature_manager.InsertScale(
                            0,  # swScaleType_e.swScaleAboutCentroid
                            False,  # 统一缩放
                            self.scale_factor,  # X 方向
                            self.scale_factor,  # Y 方向  
                            self.scale_factor   # Z 方向
                        )
                        
                        if scale_feature:
                            print(f"        ✓ 缩放成功 ({self.scale_factor}x)")
                            success_count += 1
                        else:
                            print(f"        ✗ 缩放特征创建失败")
                        ...
                        try:
                            # 获取变换矩阵
                            math_utility = self.sw_app.GetMathUtility()
                            
                            # 创建缩放矩阵 (对角线为缩放因子)
                            transform_data = [
                                self.scale_factor, 0, 0, 0,
                                0, self.scale_factor, 0, 0,
                                0, 0, self.scale_factor, 0,
                                0, 0, 0, 1
                            ]
                            ...
                            # 应用变换
                            result = body.ApplyTransform(transform)
                            
                            if result:
                                print(f"        ✓ 缩放成功 (矩阵变换 {self.scale_factor}x)")
                                success_count += 1
                            else:
                                print(f"        ✗ 变换应用失败")
                                
                        except Exception as matrix_error:
                            print(f"        ✗ 缩放失败: {safe_str(matrix_error)}")
                        
                except Exception as body_error:
                    print(f"      处理实体 {i+1} 时出错: {safe_str(body_error)}")
            
            if success_count > 0:
                return True, f"成功缩放 {success_count}/{body_count} 个实体 (倍数: {self.scale_factor})"
            else:
                return False, "未能成功缩放任何实体"
...
def main():
    print("=" * 60)
    print("SolidWorks 批量处理工具 - 支持自定义缩放")
    print("=" * 60)
    ...
    print(f"\n{'='*60}")
    print(f"处理配置：")
    print(f"  文件数量: {len(files_to_process)} 个")
    ...
    # 连接SW - 传入缩放参数
    processor = SolidWorksBatchProcessor(scale_factor=scale_factor)
